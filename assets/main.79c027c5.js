        ...
        `,[{id:"buy-click",icon:"👆",title:"Увеличить клик",description:"+1 к доходу за клик",price:this.upgradePrices.click,action:()=>this.buyUpgrade("click")},{id:"buy-auto",icon:"🤖",title:"Автоклик",description:"+1 к автоматическому доходу",price:this.upgradePrices.auto,action:()=>this.buyUpgrade("auto")},{id:"rocket-boost",icon:"🚀",title:"Ракетный ускоритель",description:"Увеличивает доход за клик в 2 раза",price:100,action:()=>this.buyRocketBoost()},{id:"satellite",icon:"🛰️",title:"Спутник",description:"Автоматически добывает ресурсы",price:500,action:()=>this.buySatellite()}].forEach(i=>{const o=document.createElement("div");o.className="upgrade-item-new",o.style.cssText=`
            ...
            `,o.innerHTML=`
                ...
                <div class="upgrade-info" style="flex: 1;">
                    <h4 style="margin: 0 0 5px 0; color: white; font-size: 16px;">${i.title}</h4>
                    <p style="margin: 0; color: rgba(255, 255, 255, 0.7); font-size: 12px;">${i.description}</p>
                </div>
                <div class="upgrade-price" style="
...
`;document.head.appendChild(u);"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").then(r=>{console.log("SW registered: ",r)}).catch(r=>{console.log("SW registration failed: ",r)})});let l=null;document.addEventListener("DOMContentLoaded",()=>{l=new d,window.addEventListener("beforeunload",()=>{l&&l.saveGame()}),window.addEventListener("unload",()=>{l&&l.destroy()})});typeof c<"u"&&c.exports&&(c.exports=d)});export default S();
